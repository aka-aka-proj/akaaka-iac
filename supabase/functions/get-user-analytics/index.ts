...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!targetUserId) {
      return jsonResponse({ error: 'invalid', message: 'user_id is required' }, 400)
    }

    if (!['weekly', 'monthly', 'all'].includes(period)) {
      return jsonResponse({ error: 'invalid_period', message: "period must be 'weekly', 'monthly', or 'all'" }, 400)
    }
    ...
    if (since) {
      hostedEventsQuery = hostedEventsQuery.gte('start_time', since).lte('start_time', new Date().toISOString())
    }

    const { data: hostedEvents, error: hostedError } = await hostedEventsQuery

    if (hostedError) {
      console.error('get-user-analytics hostedEvents error', hostedError)
      return jsonResponse({ error: 'db_error', message: hostedError.message }, 500)
    }
    ...
    const hostedTags = collectTags((hostedEvents ?? []).map((e) => e.event_type))
    ...
    if (hostedEventIds.length > 0) {
      ...
      if (approvedError) {
        return jsonResponse({ error: 'db_error', message: approvedError.message }, 500)
      }

      const { count: rejectedCount, error: rejectedError } = await serviceClient
        .from('event_registrations')
        .select('id', { count: 'exact', head: true })
        .in('event_id', hostedEventIds)
        .eq('status', 'rejected')
      ...
      if (rejectedError) {
        return jsonResponse({ error: 'db_error', message: rejectedError.message }, 500)
      }
      ...
      if (pendingError) {
        return jsonResponse({ error: 'db_error', message: pendingError.message }, 500)
      }

      const { count: waitlistConvCount, error: wlConvError } = await serviceClient
        .from('event_registrations')
        .select('id', { count: 'exact', head: true })
        .in('event_id', hostedEventIds)
      ...
      if (wlConvError) {
        return jsonResponse({ error: 'db_error', message: wlConvError.message }, 500)
      }

      const { count: checkedInCount, error: checkedInError } = await serviceClient
        .from('event_registrations')
        .select('id', { count: 'exact', head: true })
        .in('event_id', hostedEventIds)
      ...
      if (checkedInError) {
        return jsonResponse({ error: 'db_error', message: checkedInError.message }, 500)
      }
    ...
    const approvalDenominator = totalApproved + totalRejected + totalPending
    const approvalRate = approvalDenominator > 0 ? Math.round((totalApproved / approvalDenominator) * 100) : 0

    // 出席率：Checked-in / Approved
    const attendanceRate = totalApproved > 0 ? Math.round((checkedInRegistrations / totalApproved) * 100) : 0
    ...
    if (since) {
      ...
      if (rangeError) {
        return jsonResponse({ error: 'db_error', message: rangeError.message }, 500)
      }
    ...
    if (effectiveEventScope !== null) {
      if (effectiveEventScope.length > 0) {
        participatedQuery = participatedQuery.in('event_id', effectiveEventScope)
        approvedParticipationsQuery = approvedParticipationsQuery.in('event_id', effectiveEventScope)
      } else {
        const stats = await buildStats(serviceClient, targetUserId, hostedEventsCount, hostedTags, totalRegistrations, totalApproved, approvalRate, waitlistConversions, checkedInRegistrations, attendanceRate, 0, 0, [])
        return jsonResponse({ success: true, user_id: targetUserId, period, stats }, 200)
      }
    }

    const { count: eventsParticipated, error: participatedError } = await participatedQuery
    if (participatedError) {
      return jsonResponse({ error: 'db_error', message: participatedError.message }, 500)
    }

    const { count: approvedParticipations, error: approvedPartError } = await approvedParticipationsQuery
    if (approvedPartError) {
      return jsonResponse({ error: 'db_error', message: approvedPartError.message }, 500)
    }

    // --- 社群信用：加權信譽積分 + 被檢舉頻率 ---
    const reputationGained = await computeWeightedReputation(serviceClient, targetUserId)
...
function collectTags(eventTypes: (string | null)[]): string[] {
  const tags = new Set<string>()
  for (const et of eventTypes) {
    if (!et) continue
    for (const tag of et.split(',')) {
...
async function computeWeightedReputation(
  ...
  return (recommendations ?? []).reduce((sum, r) => {
    const role = r.from_profile?.role_status as string | undefined
...
async function computeExploredTags(
  ...
  return collectTags((data ?? []).map((row) => {
    const event = row.events as { event_type: string } | null
