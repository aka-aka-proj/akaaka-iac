...
export async function createProviderKey(managementKey: string, name: string, limit: number, limitReset: string) {
...
export async function deleteProviderKey(managementKey: string, hash: string) {
  await fetch(`${OPENROUTER_KEYS_URL}/${encodeURIComponent(hash)}`, {
    method: 'DELETE',
